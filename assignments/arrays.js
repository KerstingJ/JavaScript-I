...
const car33 = inventory.filter(x => x.id === 33)[0];
...
// for (let i=0; i< inventory.length; i++){
//     if (!carModels.includes(inventory[i].car_model)) carModels.push(inventory[i].car_model);
// }

inventory.forEach(x => {
    if (!carModels.includes(x.car_model)){
        carModels.push(x.car_model);
    }
});
...
console.log(`there are ${oldCars.length} cars with a year older than 2000`); 
